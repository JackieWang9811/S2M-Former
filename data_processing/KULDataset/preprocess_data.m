...
for subject = subjects
    ...
    preproc_trials = {};
    % Process only the first 8 trialss
    num_trials = min(8, size(trials, 2));
    ...
    for trialnum = 1:num_trials
        ...
        % Normalize each channel (mean = 0, variance = 1)
%         trial.RawData.EegData = (trial.RawData.EegData - mean(trial.RawData.EegData, 1)) ./ std(trial.RawData.EegData, 0, 1);
       
        % Keep only the first 46080 samples
        trial.RawData.EegData = trial.RawData.EegData(1:46080, :);
...
Fs = params.intermediateSampleRate;
% 过渡带参数（适应中间采样率）
Fst1 = max(0.05, params.highpass - 0.05); % 高�?�阻带截止频率（�?�?0.05Hz�?
Fp1 = params.highpass;                    % 高�?��?�带截止频率�?0.1Hz�?
Fp2 = params.lowpass;                     % 低�?��?�带截止频率�?50Hz�?
Fst2 = params.lowpass + 1;                % 低�?�阻带截止频率（51Hz�?
...
BP = fdesign.bandpass('Fst1,Fp1,Fp2,Fst2,Ast1,Ap,Ast2',...
