...
def extract_and_save_eeg_data(subject_number, mat_file_path, output_folder, ConType="No"):
    ...
    for ID in range(len(IDs)):
        ...
        filename = f"{output_folder}/{subject_number}Tra{ID + 1}.csv"
...
def main():
    ...
    data_document_path = "/data/wjq/AAD/AVGCDataset/preprocessed_data_zscore"
    output_data_document_path = "/data/wjq/AAD/AVGCDataset_across_conditions_zscore"
    ConType = "No_vanilla_128"
    for subject_number in subjects:
        name = f"0{subject_number}" if subject_number < 10 else f"{subject_number}"
        mat_file_path = os.path.join(data_document_path, f"2024-AV-GC-AAD-sub{name}_preprocessed_zscore.mat")
