...
% decoder estimation
params.Leeg = [0,0.4]; % integration window ([0,400ms], post-stimulus)
...
params.cv.nbFoldsOuter = 10; % number of folds to evaluate decoders in outer CV loop
...
cond = cell(length(params.subjects),length(params.windowLengths)); % all conditions
...
for s = 1:length(params.subjects)
    ...
    switch params.cv.type
        ...
        otherwise
            [eegTrials,audioTrials,attSpeaker,conditions,fs] = loadDataGazeControl(testS,params.preprocessing,max(params.windowLengths),params.condition,params.datapath);
    end
