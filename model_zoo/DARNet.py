...
class TokenEmbedding(nn.Module):
    def __init__(self, c_in, d_model): # c_in = 64, d_model = 16
    ...
    def forward(self, x):
        ...
        x = x.unsqueeze(1)  # (32,64,128) => (32,1,64,128)
        x = self.embed_layer(x) # (32,1,64,128) => (32,64,64,128)
        x = self.embed_layer2(x) # (32,64,64,128) => (32,16,1,128)
        x = x.squeeze(2)
        x = x.permute(0, 2, 1) # (32,128,16)
...
class DARNet(nn.Module):
    def __init__(self, args):
        ...
        channel_size, seq_len = args.data_shape[1], args.data_shape[2]
        d_model = 16
        emb_size = d_model
        num_heads = 8
...
if __name__ == '__main__':
    ...
    args.data_shape[1], args.data_shape[2] = 64, 256

    # 初始化模型
    model = DARNet(args).to(device)

    print(model)
    tem_tensor = torch.rand((32, 64, 256)).to(device)  # (batch_size, csp_dim, sample_points)
    # 前向传播
    output = model(tem_tensor)
    # print(output)
    print("Output shape:", output.shape)
    param_m = count_parameters(model) / 1e6  # 除以百万单位换算系数
    print("Model size: {:.2f}M".format(param_m))  # Model size: 0.07M
