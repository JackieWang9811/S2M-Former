...
class TABNet(nn.Module):
    ...
    def forward(self, eeg):
        # x = torch.squeeze(eeg, dim=1) # (32,1, 32,256)=>(32, 32,256)
        x = eeg.permute(2, 0, 1) # (32(batch size),32(CSP dim),256(window size)) => (256(window size),32(batch size), 32(CSP dim)) ??
        x = self.cm_attn(x) # (256,32,32)
        x = x.permute(1, 2, 0) # (32,32,256)
        x = self.conv(x) # conv (kernel:7, out_channels=64 ) : (32,32,256)=> (32,64,256)=> Relu+ AdaptiveAvgPool1d => (32,64,1)
        x = self.flat(x) # (32,64,1) => (32,64)
        x = self.linear1(x) # (32,64) => (32,16)
        x = F.sigmoid(x)
        x = F.dropout(x, p=0.8, training=self.training)
        x = self.linear2(x) # (32,16) => (32,4)
        x = F.relu(x)
        return x
...
class ConvLayer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, bias=False):
        ...
        # 表示卷积核大小为 (1, kernel_size, kernel_size)，即在空间维度上进行卷积，但在深度维度上保持为 1；
        # (1, stride, stride)，表示在空间维度上的步长为 stride（深度维度上的步长为 1）；
        # (0, padding, padding)，表示在空间维度上的填充为 padding（深度维度上的填充为 0）。
...
class FRBNet(nn.Module):
    def __init__(self):
        ...
        self.conv1 = ConvLayer(1, 32, 7, stride=2, padding=3) # (input channels, output channels, kernel_size(1,7,7):在空间维度上卷积，但深度维度保持为1)
        ...
        self.linear = nn.Linear(8, 2)
    def forward(self, x):
        # x：(32, 1, 5, 32, 32) => (bs, input_channel, depths, widths, heights) => conv1 => (32, 32, 5, 16, 16)
        x = self.conv1(x) #  (32, 32, 5, 16, 16)
        x = F.tanh(x)
        x = x.reshape(x.shape[0], -1, x.shape[3],  x.shape[4])  # (32, 32, 5, 16, 16) => (32, 64*5=160 , 16, 16)
        x = self.pool1(x) # (32,160,16,16) => (32,160,8,8)
        x = x.reshape(x.shape[0], 32, -1, x.shape[2], x.shape[3]) # (32,32,5,8,8)
        x = self.layer1(x) # (32,32,5,8,8) => (32,64,5,8,8)
        x = self.conv2(x) # (32,64,5,8,8) => (32,64,5,8,8)
        x = self.bn2(x)
        x = F.relu(x)
        x = self.layer2(x) # (32,64,5,8,8) => (32, 128, 5, 4, 4)
        x = self.conv3(x)
        x = self.bn3(x)
        x = F.relu(x)
        x = self.layer3(x) # (32, 128, 5, 4, 4) => (32, 256, 5, 2, 2)
        x = self.conv5(x) #  (32, 256, 5, 2, 2) => (32, 4 ,5 ,2, 2)
        x = self.bn5(x)
        x = F.relu(x)
        x = self.avgpool(x) # (32, 4 ,5 ,2, 2) => (32, 4 ,1 ,1, 1)
        x = F.relu(x)
        x = torch.flatten(x, start_dim=1) # (32,4,1,1,1) => (32,4*1*1*1) =>(32,4)
        return x
...
if __name__ == '__main__':
    ...
    # 初始化模型
    model = DBPNet(args.csp_dim).to(device)

    print(model)
    tem_tensor = torch.rand((32, 32, 256)).to(device)  # (batch_size, csp_dim, sample_points)
    fre_tensor = torch.rand([32, 1, 5, 32, 32]).to(device)  # (batch_size, csp_dim, sample_points)
    # 前向传播
    output = model(tem_tensor, fre_tensor)
    # print(output)
    print("Output shape:", output.shape)
    param_m = count_parameters(model) / 1e6  # 除以百万单位换算系数
    print("Model size: {:.2f}M".format(param_m))  # Model size: 0.88M